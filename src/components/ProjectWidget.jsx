const ProjectWidget = ({ imgSrc }) => {
  return (
    <div className="project-thumbnail-container">
      <div className="project-thumbnail-browser-header">
        <div className="project-thumbnail-browser-circles">
          <div className="c"></div>
          <div className="c"></div>
          <div className="c"></div>
        </div>

        <div className="project-thumbnail-browser">
          <div className="project-thumbnail-chevrons">
            <svg
              viewBox="0 0 20 20"
              height="16"
              width="16"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                transform="translate(6.25 3.75)"
                d="M0,6.25,6.25,0l.875.875L1.75,6.25l5.375,5.375L6.25,12.5Z"
                id="Fill"
              ></path>
            </svg>
            <svg
              viewBox="0 0 20 20"
              height="16"
              width="16"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                transform="translate(6.625 3.75)"
                d="M7.125,6.25.875,12.5,0,11.625,5.375,6.25,0,.875.875,0Z"
              ></path>
            </svg>
          </div>
          <div className="project-thumbnail-search-bar">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="7.89"
              height="7.887"
              viewBox="0 0 16.89 16.887"
            >
              <path
                d="M16.006,16.887h0l-4.743-4.718a6.875,6.875,0,1,1,.906-.906l4.719,4.744-.88.88ZM6.887,1.262a5.625,5.625,0,1,0,5.625,5.625A5.631,5.631,0,0,0,6.887,1.262Z"
                transform="translate(0.003 0)"
              ></path>
            </svg>
            localhost
          </div>
        </div>
      </div>

      <div className="project-thumbnail">
        <img src={imgSrc} />
        <div className="project-overlay-text">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="3em"
            height="3em"
            viewBox="0 0 20 20"
          >
            <path
              fill="#EBEBEB"
              d="M5 3h14v11h-2v2h-2v2H1V7h2V5h2zm13 10V4H6v9zm-3-4c-1.1 0-2-.9-2-2s.9-2 2-2s2 .9 2 2s-.9 2-2 2m1 6v-1H5V6H4v9zM7 6l10 6H7zm7 11v-1H3V8H2v9z"
            />
          </svg>
        </div>
      </div>
    </div>
  );
};
export default ProjectWidget;
